...
for sp=1:1
    ...
    for circ_point=1:circle_points_number
            ...
            % численное решение по явной схеме Эйлера
            for n=1:N
    ...
    ylabel('x1');
    ylim(scr_limit)
    title(strcat('Динамика системы по x1',' и точки покоя ',sp_string,' u=(',Ux,',',Uy,')'));
    hold off;
    ...
    xlabel('t');
    ylabel('x2');
    title(strcat('Динамика системы по x2',' и точки покоя ',sp_string,' u=(',Ux,',',Uy,')'));
    hold off;
    ...
    ylabel('x2');
    plot(xS,yS,'r*','MarkerSize',12);% маркировка стационарной точки на фазовом портрете        
    title(strcat('Фазовый портрет системы',' и точки покоя ',sp_string,' u=(',Ux,',',Uy,')'));
    hold off;

    figure(sp*4+4);
    ylim(scr_limit)
    xlim(scr_limit)
    title(strcat('система по времени t',' и точки покоя ',sp_string,' u=(',Ux,',',Uy,')'));
    xlabel('x1');
    ylabel('x2');
    zlabel('t');
    hold off;
    
    fprintf('поток для точки %d completed. last x,y=%d,%d, first x,y=%d,%d\n\n',sp,x1(n),x2(n),x1(1),x2(1));
